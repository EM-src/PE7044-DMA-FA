...
/*Setup File for DDL, DMA FINAL ASSIGNMENT PART1*/
...
CREATE TABLE AGENT (
  ...
  CREATE TABLE STAFF (
    ...
    IS_MANAGER          CHAR(1),
      CONSTRAINT STAFF_MANAGER_CK CHECK (IS_MANAGER IN ('Y', 'N')),
  ...
  CREATE TABLE CUST_TO_BRANCH (
    CUSTOMER_ID         NUMBER(3) NOT NULL
      CHECK (CUSTOMER_ID > 0),
    BRANCH_ID           NUMBER(3)  NOT NULL
      CHECK (BRANCH_ID > 0),
    CONSTRAINT PK_CUST_TB PRIMARY KEY (CUSTOMER_ID, BRANCH_ID),
    CONSTRAINT FK1_CUST_TB FOREIGN KEY (CUSTOMER_ID) REFERENCES CUSTOMER (CUSTOMER_ID),
    CONSTRAINT FK2_CUST_TB FOREIGN KEY (BRANCH_ID) REFERENCES BRANCH (BRANCH_ID)
  );
  ...
  CREATE TABLE PROPERTY (
    ...
    STATUS              VARCHAR(9) NOT NULL
      CONSTRAINT STATUS_CHK CHECK (STATUS IN ('SOLD', 'RENTED', 'AVAILABLE')),
    CONSTRAINT FK1_PROP FOREIGN KEY (OWNER_ID) REFERENCES CUSTOMER (CUSTOMER_ID),
    CONSTRAINT FK2_PROP FOREIGN KEY (BRANCH_ID) REFERENCES BRANCH (BRANCH_ID),
    CONSTRAINT NOT_NULL_PRICE CHECK ((SELLING_PRICE IS NULL OR RENTAL_PRICE IS NULL)
                                      AND NOT(SELLING_PRICE IS NULL AND RENTAL_PRICE IS NULL))
  );
  ...
  CREATE TABLE CUSTOMER_INTENTION (
    ...
    PURPOSE             CHAR(5) NOT NULL
      CONSTRAINT PURPOSE_CHK CHECK (PURPOSE IN ('BUY', 'RENT', 'SELL', 'LEASE')),
    CONSTRAINT FK_CUST_INTENT FOREIGN KEY (CUSTOMER_ID) REFERENCES CUSTOMER (CUSTOMER_ID),
    CONSTRAINT PK_CUST_INENT PRIMARY KEY (CUSTOMER_ID, PURPOSE)
  );
  ...
  CREATE TABLE PROPERTY_SOLD (
    ...
    PROPERTY_ID         NUMBER(3) NOT NULL
      CHECK (PROPERTY_ID > 0)
      CONSTRAINT PROP_S_UNIQUE_CK UNIQUE,
    SELLING_PRICE       NUMBER(9,2) NOT NULL
      CONSTRAINT SP_POS CHECK (SELLING_PRICE > 0),
    STAMP_DUTY          NUMBER(7,2) AS (SELLING_PRICE * 0.06)
      CONSTRAINT SD_POS CHECK (STAMP_DUTY > 0),
    COMMISSION          NUMBER(8,2) AS (SELLING_PRICE * 0.01)
  ...
  CREATE TABLE PROPERTY_RENTED (
    ...
    PROPERTY_ID         NUMBER(3) NOT NULL
      CHECK (PROPERTY_ID > 0)
      CONSTRAINT PROP_R_UNIQUE_CK UNIQUE,
    LANDLORD_ID         NUMBER(3) NOT NULL
      CHECK (LANDLORD_ID > 0),
    RENT_PER_MONTH      NUMBER(6,2) NOT NULL
      CHECK (RENT_PER_MONTH > 0),
    DEPOSIT             NUMBER(7,2) AS (RENT_PER_MONTH * 3),
    COMMISSION          NUMBER(7,2) AS (RENT_PER_MONTH * 1.5),
    START_DATE          DATE,
    END_DATE            DATE,
  ...
  CREATE TABLE VIEWINGS (
    PROPERTY_ID         NUMBER(3) NOT NULL
      CHECK (PROPERTY_ID > 0)
      CONSTRAINT FK1_VIEW REFERENCES PROPERTY,
    CUSTOMER_ID         NUMBER(3) NOT NULL
      CHECK (CUSTOMER_ID > 0)
